---
title: "Terms of Use"
description: "PhotoSifter Terms of Use and License Agreement"
layout: "single"
---

<section class="bg-gradient-to-br from-blue-600 to-indigo-700 text-white py-16">
  <div class="container mx-auto px-6 lg:px-8">
    <h1 class="text-4xl md:text-5xl font-bold mb-4">Terms of Use</h1>
    <p class="text-xl text-blue-100">Last updated: December 2024</p>
  </div>
</section>

<section class="py-16 bg-white">
  <div class="container mx-auto px-6 lg:px-8 max-w-4xl">
    <div class="prose prose-lg max-w-none">

## Agreement

By downloading, installing, or using PhotoSifter, you agree to these terms. If you don't agree, please don't use the software.

## License

### Free Version
The free version of PhotoSifter is licensed for personal use with the following limitations:
- Process up to 150 photos per session
- All core features included
- No time limit

### Pro License
The Pro license is a one-time purchase that grants you:
- Unlimited photo processing
- Lifetime updates
- Priority support
- Use on up to 3 personal devices

### Restrictions
You may NOT:
- Redistribute, sell, or sublicense the software
- Reverse engineer or decompile the software
- Remove or alter any proprietary notices
- Use the software for illegal purposes
- Share your license key with others

## Intellectual Property

PhotoSifter and all associated materials are the property of PhotoSifter. The software is protected by copyright and other intellectual property laws.

## Disclaimer of Warranties

PhotoSifter is provided "AS IS" without warranty of any kind. We do not guarantee that:
- The software will meet your requirements
- The software will be error-free or uninterrupted
- Results from the software will be accurate

**Always maintain backups of your photos.** While PhotoSifter is designed to safely organize your photos, we are not responsible for any data loss.

## Limitation of Liability

To the maximum extent permitted by law, PhotoSifter shall not be liable for any indirect, incidental, special, or consequential damages arising from your use of PhotoSifter.

Our total liability shall not exceed the amount you paid for the software (if any).

## Refunds

Pro license purchases are eligible for a refund within 14 days if:
- You have not used the license key
- You contact us at support@photosifter.com

## Termination

We may terminate your license if you violate these terms. Upon termination, you must stop using the software and delete all copies.

## Updates

We may update these terms from time to time. Continued use of PhotoSifter after changes constitutes acceptance of the new terms.

## Governing Law

These terms are governed by the laws of the United States. Any disputes shall be resolved in the courts of California.

## Contact

Questions about these terms? Contact us:
- Email: [support@photosifter.com](mailto:support@photosifter.com)

</div>
  </div>
</section>
